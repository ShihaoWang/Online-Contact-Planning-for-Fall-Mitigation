...
protected:
	std::vector<SplineVector> _way_points;
public:
	void add_way_point(const SplineVector& point);
	void clear();

protected:
	void add_node(const SplineVector& node);
	virtual void _on_way_point_added()=0;

protected:
	std::vector<SplineVector> _nodes;
	std::vector<double> _distances;
public:
	SplineVector node(int i) const { return _nodes[i]; }
