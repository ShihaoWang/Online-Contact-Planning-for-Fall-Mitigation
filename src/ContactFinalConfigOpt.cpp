...
struct ContactFinalConfigOpt: public NonlinearOptimizerInfo
{
  // This is the class struct for the optimizaiton of configuraiton variables

  ContactFinalConfigOpt():NonlinearOptimizerInfo(){};
  ...
  static std::vector<double> InitialConfigObjNCons(const int & nVar, const int & nObjNCons, const std::vector<double> & ConfigOpt)
  {
    ...
    // First idea is to enable a small configuration deviation.
    ...
    // Second concern is to make COM projection into the center of SP.
    ...
    F[0] =  ConfigVia - COMDist;
    return F;
  }
};

static std::vector<double> ContactFinalConfigOptFn(std::vector<double> &RobotConfig)
{
  // This function is used to optimize the robot configuraiton variable such that certain contact is active.
  ContactFinalConfigOpt ContactFinalConfigOptProblem;
  ...
  ContactFinalConfigOptProblem.InnerVariableInitialize(n, neF);
  ...
  */
  ...
  ContactFinalConfigOptProblem.VariableBoundsUpdate(xlow_vec, xupp_vec);
  ...
  */
  ...
  ContactFinalConfigOptProblem.ConstraintBoundsUpdate(Flow_vec, Fupp_vec);

  /*
    Initialize the seed guess
  */
  ContactFinalConfigOptProblem.SeedGuessUpdate(RobotConfigRef);

  /*
    Given a name of this problem for the output
  */
  ContactFinalConfigOptProblem.ProblemNameUpdate("ContactFinalConfigOptProblem", 0);

  ContactFinalConfigOptProblem.NonlinearProb.setIntParameter("Iterations limit", 10000);
  ContactFinalConfigOptProblem.NonlinearProb.setIntParameter("Major iterations limit", 25);
  ContactFinalConfigOptProblem.NonlinearProb.setIntParameter("Major print level", 0);
  ContactFinalConfigOptProblem.NonlinearProb.setIntParameter("Minor print level", 0);
  /*
    ProblemOptions seting
  */
  // Solve with Finite-Difference
  ContactFinalConfigOptProblem.ProblemOptionsUpdate(0, 3);
  ContactFinalConfigOptProblem.Solve(RobotConfig);

  return RobotConfig;
}

std::vector<double> ContactFinalConfigOptimization(Robot& _SimRobotObj, const int & SwingLimbIndex, const std::vector<ContactStatusInfo> &  _RobotContactInfo, const std::vector<double>& _RobotConfigRef)
{
  SimRobotObj = _SimRobotObj;
  RobotContactInfo = _RobotContactInfo;
  for(int i = 0; i<RobotContactInfo[SwingLimbIndex].LocalContactStatus.size(); i++)
  {
    RobotContactInfo[SwingLimbIndex].LocalContactStatus[i] = 1;
  }

  RobotConfigRef = _RobotConfigRef;

  std::vector<double> RobotConfig(_RobotConfigRef.size(), 0);
  return ContactFinalConfigOptFn(RobotConfig);
}
